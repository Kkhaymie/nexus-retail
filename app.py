...
_defaults = {
    ...
    "executive_brief": None, "executive_brief_date": None, "cust_df": None, "kpi_summary": {},
    "alerts": [], "scheduler_started": False, "email_result": None,
    "prev_kpis": {}, "mistral_api_key": os.getenv("MISTRAL_API_KEY", ""),
    "theme": "dark",
    # Email config — seeded from .env if present, editable in UI
...
section[data-testid="stSidebar"] [data-baseweb="popover"] li {{
    color: #F1F5F9 !important; background: #1E1B4B !important;
}}
...
section[data-testid="stSidebar"] .stSlider [role="slider"] {{
    background: #A78BFA !important; border: 2px solid #7C3AED !important;
}}
...
section[data-testid="stSidebar"] [data-testid="stFileUploader"] {{
    ...
    border: 1px dashed #3730A3 !important; border-radius: 10px !important;
...
.eyebrow {{
    ...
    text-transform: uppercase; color: {T['primary_soft']}; margin-bottom: 4px;
}}
.page-title {{
    font-family: 'Space Grotesk', sans-serif; font-size: 2rem; font-weight: 700;
    color: {T['text']} !important; letter-spacing: -0.5px; margin-bottom: 4px;
}}
.page-sub {{ font-size: 0.86rem; color: {T['text2']}; margin-bottom: 1.4rem; }}

.kpi-card {{
    background: {T['surface']}; border: 1px solid {T['border']};
    border-radius: 14px; padding: 18px 16px 14px; position: relative;
    overflow: hidden; transition: transform 0.15s, box-shadow 0.15s;
    min-height: 130px;
}}
.kpi-card::before {{
    content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px;
    background: linear-gradient(90deg, {T['primary']}, {T['primary_soft']});
}}
...
.section-label {{
    ...
    text-transform: uppercase; color: {T['primary_soft']}; margin-bottom: 0.75rem;
}}

.insight-tile {{
    background: {T['surface']}; border: 1px solid {T['border']};
    border-radius: 12px; padding: 16px 12px; text-align: center; min-height: 110px;
}}
.insight-val {{ font-family:'Space Grotesk',sans-serif; font-size:1.2rem; font-weight:700; }}
.insight-lbl {{ font-size:0.66rem; font-weight:700; text-transform:uppercase;
                letter-spacing:0.5px; color:{T['text2']}; margin-top:5px; }}
...
.chat-user {{
    ...
    margin:8px 0; font-size:0.875rem; color:{T['text']};
}}
.chat-ai {{
    background:rgba(15,118,110,0.09); border:1px solid rgba(15,118,110,0.22);
    padding:12px 16px; border-radius:12px 12px 12px 4px;
    margin:8px 0; font-size:0.875rem; color:{T['text']}; line-height:1.65;
}}
.report-card {{
    background:{T['surface']}; border:1px solid {T['border']};
...
with st.sidebar:
    ...
    <div style="padding:4px 0 16px;">
        ...
        <div style="font-size:0.66rem;color:#A78BFA;letter-spacing:1.8px;
                    text-transform:uppercase;margin-top:3px;">Revenue Intelligence OS</div>
        <div style="display:inline-block;background:rgba(124,58,237,0.2);
                    border:1px solid rgba(124,58,237,0.4);color:#A78BFA;
                    font-size:0.63rem;padding:2px 9px;border-radius:20px;
                    font-weight:700;margin-top:8px;">v2.0 · AI-Powered</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("🔑 API Key", expanded=not bool(st.session_state.mistral_api_key)):
        api_key_input = st.text_input(
            "Mistral API Key", value=st.session_state.mistral_api_key,
    ...
    uploaded_file = st.file_uploader(
        ...
        help="Any retail CSV or Excel — NEXUS auto-detects all columns.",
    )
    if uploaded_file:
            ...
            with st.spinner("Analysing…"):
                try:
                    ...
                    if anomalies and st.session_state.mistral_api_key:
                        ...
                        send_intelligence_email(
                            kpi_summary=summary_obj, alerts=alerts_obj,
    ...
    if st.session_state.df_with_risk is not None:
        ...
        with st.expander("🧮 Filters & Slicers", expanded=False):
            ...
            _slicer_defs = [
                ("channel",          "🛍️ Sales Channel"),
                ("customer_segment", "👥 Customer Segment"),
                ("region",           "🌍 Region"),
                ("product_category", "📦 Product Category"),
            ]
            for _key, _label in _slicer_defs:
                _col = _cmap.get(_key)
                if _col and _col in _fdf.columns:
                    ...
                    if 1 < len(_opts) <= 60:
                        ...
                        st.session_state[f"flt_{_key}"] = st.multiselect(
                            _label, _opts,
                            default=st.session_state.get(f"flt_{_key}", []),
                            label_visibility="collapsed", key=f"flt_ms_{_key}",
                            placeholder="All",
                        )

            if "Risk_Tier" in _fdf.columns:
                _risk_opts = ["High Value","Stable","At Risk","Profit Drain"]
                _risk_opts = [r for r in _risk_opts if r in _fdf["Risk_Tier"].astype(str).unique()]
                if _risk_opts:
                    ...
                    st.session_state["flt_risk_tier"] = st.multiselect(
                        "Risk Tier", _risk_opts,
                        default=st.session_state.get("flt_risk_tier", []),
                        label_visibility="collapsed", key="flt_ms_risk_tier",
                        placeholder="All",
                    )
    ...
    st.markdown("---")
    st.markdown(
        '<div style="font-size:0.68rem;color:#475569;line-height:1.75;">'
        'NEXUS Retail v2.0<br>Powered by Mistral AI<br>RRI · RVI · Email Intelligence</div>',
        unsafe_allow_html=True,
    )
...
def chart_layout():
    return dict(
        plot_bgcolor=T["cb"], paper_bgcolor=T["cb"],
        font=dict(family="Inter", color=T["ct"]),
        title_font=dict(family="Space Grotesk", color=T["text"], size=14),
        margin=dict(t=50, b=28, l=10, r=10),
        legend=dict(font_color=T["ct"]),
    )
...
<div style="padding:2.5rem 0 1.5rem;">
    <div style="font-size:0.66rem;font-weight:700;letter-spacing:2.5px;
                text-transform:uppercase;color:#A78BFA;margin-bottom:10px;">
        Revenue Intelligence Platform · Nigerian Retail
    </div>
    <div style="font-family:'Space Grotesk',sans-serif;font-size:2.9rem;
                font-weight:700;color:{T['text']};letter-spacing:-1.5px;line-height:1.05;">
    ...
    <div style="font-size:0.98rem;color:{T['text2']};margin-top:14px;
                max-width:520px;line-height:1.65;">
        Upload any retail dataset. NEXUS Retail cleans it, scores every order for risk,
        scores every customer for health, and pinpoints where money is leaking — in seconds.
    </div>
</div>
    """, unsafe_allow_html=True)

    st.markdown(f"""
<div style="display:flex;gap:40px;padding:20px 0;flex-wrap:wrap;
            border-top:1px solid {T['border']};border-bottom:1px solid {T['border']};
            margin-bottom:2.5rem;">
    <div>
        <div style="font-family:'Space Grotesk',sans-serif;font-size:1.55rem;font-weight:700;color:{T['text']};">₦68.5M</div>
        <div style="font-size:0.72rem;color:{T['text2']};margin-top:2px;">Net revenue analysed</div>
    </div>
    <div>
        <div style="font-family:'Space Grotesk',sans-serif;font-size:1.55rem;font-weight:700;color:{T['text']};">1,506</div>
        <div style="font-size:0.72rem;color:{T['text2']};margin-top:2px;">Orders processed</div>
    </div>
    <div>
        <div style="font-family:'Space Grotesk',sans-serif;font-size:1.55rem;font-weight:700;color:#F43F5E;">174</div>
        <div style="font-size:0.72rem;color:{T['text2']};margin-top:2px;">Loss-making orders</div>
    </div>
    <div>
        <div style="font-family:'Space Grotesk',sans-serif;font-size:1.55rem;font-weight:700;color:#A78BFA;">339</div>
        <div style="font-size:0.72rem;color:{T['text2']};margin-top:2px;">Customers scored</div>
    </div>
    <div>
        <div style="font-family:'Space Grotesk',sans-serif;font-size:1.55rem;font-weight:700;color:{T['teal']};">2</div>
        <div style="font-size:0.72rem;color:{T['text2']};margin-top:2px;">Derived metrics</div>
    </div>
</div>
    """, unsafe_allow_html=True)
    ...
    for i, (icon, title, desc, accent) in enumerate(features):
        c = T["primary"] if accent=="purple" else T["teal"]
        with cols[i % 3]:
            st.markdown(f"""
<div class="feature-card" style="margin-bottom:12px;">
    <div style="font-size:1.8rem;margin-bottom:10px;">{icon}</div>
    <div style="font-family:'Space Grotesk',sans-serif;font-weight:700;
                font-size:0.9rem;color:{T['text']};margin-bottom:6px;">{title}</div>
    <div style="font-size:0.8rem;color:{T['text2']};line-height:1.55;">{desc}</div>
</div>""", unsafe_allow_html=True)
...
<div style="background:rgba(124,58,237,0.08);border:1px solid rgba(124,58,237,0.28);
    ...
    <div style="font-weight:700;font-size:0.86rem;color:#A78BFA;margin-bottom:8px;">
        Get started in 3 steps
    </div>
    <div style="font-size:0.82rem;color:{T['text2']};line-height:1.85;">
        1 · Add your Mistral API key in the sidebar — free at
        <a href="https://console.mistral.ai" target="_blank" style="color:#7C3AED;">console.mistral.ai</a><br>
...
if _active_filters:
    _chips = "".join(
        ...
        f'border:1px solid rgba(124,58,237,0.35);color:{T["primary_soft"]};'
        f'font-size:0.72rem;font-weight:600;padding:3px 10px;border-radius:20px;'
        f'margin:0 6px 6px 0;">{c}</span>'
        for c in _active_filters
    )
    st.markdown(
        f'<div style="margin-bottom:14px;">'
        f'<span style="font-size:0.66rem;font-weight:700;letter-spacing:1.5px;'
        f'text-transform:uppercase;color:{T["text2"]};margin-right:8px;">🧮 Active Filters</span>'
        f'{_chips}'
        f'<span style="font-size:0.72rem;color:{T["text2"]};margin-left:4px;">· {len(df):,} of {len(st.session_state.df_with_risk):,} rows</span>'
...
if page == "🏠 Command Center":
    ...
    with k1:
        kpi_card("","Total Orders", f"{tot_orders:,}",
                 "Dataset records","b-purple","FY 2025")
    with k2:
        kpi_card("","Net Revenue", fmt_ngn(net_rev),
                 "After returns & discounts","b-purple",f"Gross {fmt_ngn(gross_rev)}")
    with k3:
        m_acc = "t-ok" if (margin or 0)>15 else "t-warn"
        m_bdg = "b-ok" if (margin or 0)>15 else "b-warn"
        m_txt = "✓ Above 20%" if (margin or 0)>15 else "⚠ Below 20%"
        kpi_card(m_acc,"Profit Margin", fmt_pct(margin),
                 f"Total profit {fmt_ngn(profit)}",m_bdg,m_txt)
    with k4:
        r_acc = "t-danger" if (ret_rate or 0)>5 else "t-ok"
        r_bdg = "b-danger" if (ret_rate or 0)>5 else "b-ok"
        r_txt = "⚠ Above 5%" if (ret_rate or 0)>5 else "✓ Healthy"
        kpi_card(r_acc,"Return Rate", fmt_pct(ret_rate),
                 "Industry benchmark 7.4%",r_bdg,r_txt)
    with k5:
        rr_acc = "t-warn" if avg_rri>30 else "t-ok"
        rr_bdg = "b-warn" if avg_rri>30 else "b-ok"
        kpi_card(rr_acc,"Avg Risk Score",
                 f'{avg_rri}<span class="kpi-unit">/100</span>',
                 "Revenue Risk Index",rr_bdg,"Elevated" if avg_rri>30 else "Normal")
    with k6:
        tr_acc = "t-ok" if trust_score>=75 else "t-warn"
        tr_bdg = "b-ok" if trust_score>=75 else "b-warn"
        kpi_card(tr_acc,"Data Trust Score",
                 f'{trust_score}<span class="kpi-unit">/100</span>',
                 "Cleaning quality",tr_bdg,trust_lbl)
    ...
    with p1:
        if monthly is not None and len(monthly)>=3:
            try:
                ...
                trend = "📈" if m.coef_[0]>0 else "📉"
                insight_tile(fmt_ngn(max(nxt,0)),"Revenue Forecast",f"{trend} projected next month",T["primary_soft"])
            except:
                insight_tile("—","Revenue Forecast","Insufficient data",T["text2"])
        else:
            insight_tile("—","Revenue Forecast","Need ≥3 months",T["text2"])

    cust_live = st.session_state.get("_cust_df_view")
    with p2:
        if cust_live is not None and "Vitality_Tier" in cust_live.columns:
            total_c   = len(cust_live)
            dorm_n    = (cust_live["Vitality_Tier"]=="Dormant").sum()
            churn_pct = round(dorm_n/total_c*100,1)
            insight_tile(f"{dorm_n}","Churn Risk",
                         f"⚠ {churn_pct}% dormant",
                         T["danger"] if churn_pct>20 else T["warning"])
        else:
            insight_tile("—","Churn Risk","RVI not computed",T["text2"])

    with p3:
        if cust_live is not None and "Vitality_Tier" in cust_live.columns:
            total_c  = len(cust_live)
            champ_n  = (cust_live["Vitality_Tier"]=="Champion").sum()
            insight_tile(str(champ_n),"Champions",
                         f"🏆 {round(champ_n/total_c*100,1)}% of base",T["success"])
        else:
            insight_tile("—","Champions","RVI not computed",T["text2"])
    ...
    with p4:
        if rep_col and net_col and rep_col in df.columns and net_col in df.columns:
            ...
            rep_shr = round(rep_rev/tot_rev*100,1) if tot_rev>0 else 0
            insight_tile(fmt_pct(rep_shr),"Repeat Revenue","of total net revenue",T["teal"])
        else:
            insight_tile("—","Repeat Revenue","No repeat column",T["text2"])
    ...
    with p5:
        if chan_col and net_col and profit_col and all(c in df.columns for c in [chan_col,net_col,profit_col]):
            cm = df.groupby(chan_col).apply(
                lambda x: x[profit_col].sum()/x[net_col].sum()*100 if x[net_col].sum()>0 else 0)
            insight_tile(str(cm.idxmax()),"Best Margin Channel",
                         f"💰 {round(cm.max(),1)}% margin",T["teal"])
        else:
            insight_tile("—","Best Margin Channel","Column not found",T["text2"])
    ...
    if rep_col and date_col and net_col and all(c in df.columns for c in [rep_col,date_col,net_col]):
        ...
        fig_rn.update_layout(barmode="stack",title="Repeat vs New Customer Revenue by Month",
                              legend=dict(orientation="h",y=1.12))
        st.plotly_chart(sf(fig_rn,340),use_container_width=True)

    divider()
    section_lbl("REVENUE ANALYTICS — 6 VIEWS")

    c1,c2 = st.columns(2)
    with c1:
        d = get_revenue_by_column(df,col_map,"channel")
        if d is not None:
            fig1 = px.bar(d,x=d.columns[0],y="Total Revenue",title="Net Revenue by Sales Channel",
                          color="Total Revenue",color_continuous_scale=PS,text="Total Revenue")
    ...
    with c2:
        if monthly is not None:
            fig2 = px.area(monthly,x="Month",y="Net Revenue",title="Monthly Revenue Trend",
    ...
    with c1:
        if "Risk_Tier" in df.columns:
            ...
            tc  = {"Profit Drain":"#F43F5E","At Risk":"#F59E0B","Stable":"#A78BFA","High Value":"#10B981"}
            fig3 = px.pie(ts_,names="Risk Tier",values="Count",title="Order Revenue Risk Distribution",
                          color="Risk Tier",color_discrete_map=tc,hole=0.52)
    ...
    with c2:
        ...
        if cd is not None:
            fig4 = px.bar(cd.head(8),x="Total Revenue",y=cd.columns[0],orientation="h",
                          title="Revenue by Product Category",color="Total Revenue",color_continuous_scale=TS)
    ...
    with c1:
        ...
        if sd is not None:
            fig5 = px.bar(sd,x=sd.columns[0],y="Avg Order Value",title="Avg Order Value by Segment",
                          color="Avg Order Value",color_continuous_scale=PS,text="Avg Order Value")
            fig5.update_traces(texttemplate="₦%{text:,.0f}",textposition="outside")
            st.plotly_chart(sf(fig5),use_container_width=True)
    with c2:
        rc = get_return_rate_by_column(df,col_map,"product_category")
        if rc is not None:
            fig6 = px.bar(rc,x=rc.columns[0],y="Return Rate %",title="Return Rate by Category",
                          color="Return Rate %",color_continuous_scale=RS,text="Return Rate %")
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    page_header("Customer Intelligence","Customer Vitality",
                "Retail Vitality Index — every customer scored 0–100 for health and churn risk.")

    cust_df = st.session_state.get("_cust_df_view")
    if cust_df is None or "RVI_Score" not in cust_df.columns:
        st.info("Upload a dataset and run analysis to compute customer vitality scores.")
        st.stop()
    ...
    with v1: kpi_card("","Avg RVI Score",f'{avg_}<span class="kpi-unit">/100</span>',
                      "Customer health index","b-purple","All customers scored")
    with v2: kpi_card("t-ok","Champion",str(td_.get("Champion",0)),
                      f"{round(td_.get('Champion',0)/tot_*100,1) if tot_ else 0}% of base",
                      "b-ok","High value · protect")
    with v3: kpi_card("","Loyal",str(td_.get("Loyal",0)),
                      f"{round(td_.get('Loyal',0)/tot_*100,1) if tot_ else 0}% of base",
                      "b-purple","Stable · upsell")
    with v4: kpi_card("t-warn","Developing",str(td_.get("Developing",0)),
                      f"{round(td_.get('Developing',0)/tot_*100,1) if tot_ else 0}% of base",
                      "b-warn","Monitor · nudge")
    with v5: kpi_card("t-danger","Dormant",str(td_.get("Dormant",0)),
                      f"{round(td_.get('Dormant',0)/tot_*100,1) if tot_ else 0}% of base",
                      "b-danger","⚠ Re-engage now")
    with v6: kpi_card("t-teal","Total Customers",str(tot_),
                      "Unique customer IDs","b-teal","All scored")

    divider()
    c1,c2 = st.columns(2)
    with c1:
        vc = {"Champion":"#10B981","Loyal":"#7C3AED","Developing":"#F59E0B","Dormant":"#F43F5E"}
        fv = px.pie(ts_,names="Vitality Tier",values="Customers",title="Customer Vitality Distribution",
                    color="Vitality Tier",color_discrete_map=vc,hole=0.52)
    ...
    with c2:
        ...
        if sf_:
            ...
            if sr is not None:
                fs = px.bar(sr,x=sf_,y="Avg RVI",title="Avg RVI by Segment",
                            color="Avg RVI",color_continuous_scale=TS,text="Avg RVI")
    ...
    with c1:
        ...
        if champs is not None and len(champs)>0:
            ...
            st.dataframe(champs[[c for c in w if c and c in champs.columns]],
    ...
    with c2:
        ...
        if dorms is not None and len(dorms)>0:
            w = [col_map.get("customer_id"),col_map.get("customer_segment"),"RVI_Score","Vitality_Tier"]
            st.dataframe(dorms[[c for c in w if c and c in dorms.columns]],
                         use_container_width=True,hide_index=True)

    divider()
    section_lbl("INTERVENTIONS BY VITALITY TIER")
    for tier in ["Champion","Loyal","Developing","Dormant"]:
        with st.expander(tier):
            for action in get_vitality_intervention(tier):
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    page_header("Revenue Intelligence","Deep Analysis",
                "Cross-dimensional drill-down — segment, channel, product, and risk breakdowns.")

    dims = {}
    for label,key in {
        "Sales Channel":"channel","Product Category":"product_category",
        "Customer Segment":"customer_segment","Region":"region","City":"city",
        "Acquisition Source":"acq_source","Payment Method":"payment",
    }.items():
        if col_map.get(key) and col_map[key] in df.columns:
            dims[label] = key
    ...
    if dims:
        ...
        if dd is not None:
            ...
            with c1:
                f = px.bar(dd,x=dd.columns[0],y="Total Revenue",title=f"Revenue by {sel}",
                           color="Total Revenue",color_continuous_scale=PS,text="Total Revenue")
    ...
    divider()
    section_lbl("DISCOUNT IMPACT ON PROFIT")
    di = get_discount_impact(df,col_map)
    if di is not None:
        c1,c2 = st.columns([2,1])
        with c1:
            f7 = px.bar(di,x="Discount Band",y="Avg Profit (NGN)",title="Avg Profit by Discount Band",
                        color="Avg Profit (NGN)",color_continuous_scale=RS,text="Avg Profit (NGN)")
    ...
    divider()
    section_lbl("TOP 20 HIGHEST-RISK ORDERS")
    ...
    if tr is not None and len(tr)>0:
        def ct(v):
            return {
                "Profit Drain":"background-color:#2D1B1B;color:#F43F5E",
                "At Risk":     "background-color:#2D2410;color:#F59E0B",
                "Stable":      "background-color:#1B1F2D;color:#A78BFA",
                "High Value":  "background-color:#0F261E;color:#10B981",
            }.get(str(v),"")
        if "Risk_Tier" in tr.columns:
            st.dataframe(tr.style.map(ct,subset=["Risk_Tier"]),
                         use_container_width=True,hide_index=True)
        else:
            st.dataframe(tr,use_container_width=True,hide_index=True)

    divider()
    section_lbl("INTERVENTIONS BY RISK TIER")
    for tier in ["Profit Drain","At Risk","Stable","High Value"]:
        with st.expander(tier):
            for i,a in enumerate(get_intervention_recommendation(tier),1):
                st.markdown(f"{i}. {a}")

    divider()
    section_lbl("DATA TRUST REPORT")
    if trust_report:
        ...
        with c2:
            if trust_report.get("issues_found"):
                ...
                for iss in trust_report["issues_found"]:
                    st.markdown(f"- {iss}")
        if trust_report.get("imputation_log"):
            with st.expander("Imputation decisions"):
                for lg in trust_report["imputation_log"]:
                    st.markdown(f"- {lg}")
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    page_header("AI Intelligence","Executive Brief",
                "Structured 5-section AI brief — summary, insights, conclusion, recommendations, and next steps.")

    if not st.session_state.mistral_api_key:
        st.warning("Add your Mistral API key in the sidebar.", icon="🔑")
        st.stop()

    st.markdown(f"""
<div class="report-card">
    <div style="font-weight:700;font-size:0.86rem;color:{T['primary_soft']};margin-bottom:10px;">
        📄 Report structure — 5 sections
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;
                font-size:0.8rem;color:{T['text2']};">
        <div>01 · Executive Summary</div><div>02 · In-Depth Insights</div>
        <div>03 · Conclusion</div><div>04 · Recommendations</div>
        <div style="grid-column:1/-1;">05 · Next Steps & Action Plan</div>
    </div>
</div>""", unsafe_allow_html=True)
    ...
    if st.button("Generate Executive Brief", type="primary"):
        ...
        with st.spinner("Writing your brief…"):
            try:
                ...
                rvi_avg_ = round(cust_df_["RVI_Score"].mean(), 1) if (
                    cust_df_ is not None and "RVI_Score" in cust_df_.columns) else "N/A"
...
Today's date is {today_str}. Do NOT include a title, date line, or "Prepared by" line —
start directly with the first section header below.
...
- Write in professional English. Total length: 550-700 words."""
                ...
                # Strip any preamble before the first section marker
                first_marker_pos = brief.find("== EXECUTIVE SUMMARY ==")
                if first_marker_pos > 0:
                    brief = brief[first_marker_pos:]

                # Remove ALL markdown bold/italic markers the model might still emit
                brief = re.sub(r'\*{1,3}', '', brief)
                brief = re.sub(r'_{1,2}(.*?)_{1,2}', r'\1', brief)
                brief = brief.replace('`', '')

                # Replace any date placeholders
    ...
    if st.session_state.executive_brief:
        ...
        # ── Report header card ────────────────────────────────────────────
        st.markdown(f"""
<div class="report-card" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
    <div>
        <div style="font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:1.1rem;color:{T['text']};">NEXUS Retail Executive Brief</div>
        <div style="font-size:0.78rem;color:{T['text2']};margin-top:2px;">Prepared by NEXUS Retail Intelligence System</div>
    </div>
    <div style="font-size:0.8rem;color:{T['primary_soft']};font-weight:600;">📅 {brief_date}</div>
</div>""", unsafe_allow_html=True)
            ...
            for raw_line in lines:
                ...
                if is_subheading:
                    ...
                    html_parts.append(
                        ...
                        f'font-size:0.88rem;color:{T["primary_soft"]};margin:14px 0 6px;">'
                        f'{line}</div>'
                    )
                    continue
                ...
                close_lists()
                html_parts.append(
                    f'<p style="margin:0 0 10px 0;line-height:1.8;color:{T["text"]};">{line}</p>'
                )

            close_lists()
            return "".join(html_parts)

        # ── Section extraction and rendering ─────────────────────────────
        markers = [
        ...
        all_markers = [m for m, _ in markers]
        rendered = 0

        for marker, display in markers:
            if marker not in brief_text:
                continue
            start = brief_text.index(marker) + len(marker)
            end   = len(brief_text)
...
<div class="report-card">
    <div style="font-family:'Space Grotesk',sans-serif;font-weight:700;
                font-size:1rem;color:{T['primary_soft']};margin-bottom:12px;">{display}</div>
    <div style="font-size:0.88rem;line-height:1.8;color:{T['text']};">{content_html}</div>
</div>""", unsafe_allow_html=True)
            rendered += 1

        # Fallback: render everything as one card if no markers matched
        if rendered == 0:
            st.markdown(
                f'<div class="report-card" style="font-size:0.88rem;line-height:1.8;">'
        ...
        st.download_button(
            "📥 Download Brief as Text", data=brief_text,
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    page_header("Automated Delivery", "Email Intelligence",
                "Monday 8AM stakeholder briefings and instant anomaly alerts.")
    ...
    with cfg_c1:
        ...
        new_password = st.text_input(
            ...
            key="ei_password_input",
            help="Gmail → Google Account → Security → App Passwords. NOT your login password.",
        )
    with cfg_c2:
    ...
    if st.button("💾 Save Email Config", type="primary", key="ei_save"):
        ...
        # Push into env so scheduler + send functions pick them up immediately
        os.environ["EMAIL_SENDER"]   = st.session_state.email_sender
        os.environ["EMAIL_PASSWORD"] = st.session_state.email_password
        _recs_save = [
            r.strip() for r in
            st.session_state.email_recipients.replace("\n", ",").split(",")
            if r.strip()
        ]
        os.environ["EMAIL_RECIPIENTS"] = ",".join(_recs_save)
        st.success(f"✅ Config saved — {len(_recs_save)} recipient(s) configured.", icon="✅")
        st.rerun()

    divider()

    # ── Live status cards (reflect current session state) ─────────────────
    ...
    statuses = [
        ("Sender email", _sender_ok,   st.session_state.email_sender or "Not configured"),
        ("App password", _password_ok, "Configured ✓" if _password_ok else "Not configured"),
        ("Recipients",   _recs_ok,     f"{len(_recs_live)} address(es) configured" if _recs_ok else "Not configured"),
    ]
    sc1, sc2, sc3 = st.columns(3)
    for col, (label, ok, detail) in zip([sc1, sc2, sc3], statuses):
        bc = T["teal"] if ok else T["danger"]
        col.markdown(f"""
<div style="background:{T['surface']};border:1px solid {bc};border-left:4px solid {bc};
            border-radius:10px;padding:14px 16px;">
    <div style="font-weight:700;font-size:0.82rem;color:{bc};">{'✓' if ok else '✗'} {label}</div>
    <div style="font-size:0.74rem;color:{T['text2']};margin-top:3px;">{detail}</div>
</div>""", unsafe_allow_html=True)

    if _recs_live:
        st.markdown(
            f'<div style="font-size:0.78rem;color:{T["text2"]};margin-top:8px;">'
            f'Recipients: {" · ".join(_recs_live)}</div>',
            unsafe_allow_html=True,
        )

    divider()
    st.info(
        "**Monday briefings:** Every Monday 8:00 AM Africa/Lagos — fires automatically while the app runs.\n\n"
        "**Anomaly alerts:** Return rate > 10%, any category margin < 5%, or Profit Drain > 15% of orders."
    )

    divider()
    section_lbl("SEND TEST BRIEF")
    ...
    if not _all_configured:
        st.warning("Fill in and save the email configuration above before sending.", icon="⚠️")

    if st.button("Send Intelligence Email Now", type="primary",
                 disabled=not _all_configured, key="ei_send"):
        ksum = st.session_state.get("kpi_summary", {})
        alts = st.session_state.get("alerts", [])
        ...
        if not ksum:
            st.warning("Upload and analyse a dataset first.")
        else:
            with st.spinner("Sending…"):
                result = send_intelligence_email(
                    kpi_summary=ksum, alerts=alts,
                    ai_brief_excerpt=exc, recommended_action=rec, is_anomaly=False,
                )
            if result["success"]:
                ...
                with st.expander("Troubleshooting"):
                    st.markdown("""
- **Gmail:** App Password required — Google Account → Security → App Passwords (NOT your login password)
- **Outlook:** set `SMTP_HOST=smtp.office365.com` and `SMTP_PORT=587` in your `.env`
- Make sure you clicked **Save Email Config** above before sending
- Streamlit Cloud: add `EMAIL_SENDER`, `EMAIL_PASSWORD`, `EMAIL_RECIPIENTS` under App Secrets
                    """)
    ...
    if st.session_state.get("email_result"):
        ...
        st.markdown(
            f'<div style="font-size:0.78rem;color:{T["text2"]};margin-top:8px;">'
            f'Last sent: {r.get("message", "")} · {", ".join(r.get("recipients", []))}</div>',
            unsafe_allow_html=True,
        )
...
<div class="report-card">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;
                font-size:0.8rem;color:{T['text2']};">
        <div>✓ Data Trust Report</div>   <div>✓ Intelligence Alerts</div>
        <div>✓ Executive Summary</div>   <div>✓ In-Depth Insights</div>
        <div>✓ Recommendations</div>     <div>✓ Next Steps & Action Plan</div>
    </div>
</div>""", unsafe_allow_html=True)
    ...
    if st.session_state.df_clean is not None:
        r  = len(st.session_state.df_clean)
        cl = len(st.session_state.df_clean.columns)
        st.markdown(
            f'<div style="font-size:0.78rem;color:{T["text2"]};margin-bottom:12px;">'
            f'{r:,} rows · {cl} columns · all issues resolved</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        with c1:
            st.download_button(
                ...
                file_name=f"NEXUS_Cleaned_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv",
            )
        with c2:
            st.download_button(
                "📥 Download Excel",
                data=df_to_excel_bytes(st.session_state.df_clean),
